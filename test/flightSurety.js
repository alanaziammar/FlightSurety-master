...
contract('Flight Surety Tests', async (accounts) => {
  ...
  var config;
  var fixedtimestamp = Math.floor(Date.now() / 1000)+10000;
  ...
  /*it('(new airline) cannot register an Airline before it is funded', async () => {
    ...
    assert.equal(reverted, true, "It can register even before being funded!");  
    //let result = await config.flightSuretyData.isFunded.call(newAirline2); 
  ...
  it('(new airline) can be registered by multi consensus now', async () => {
  ...
  it('can register flight', async () => {
    ...
    // ACT
    await config.flightSuretyApp.registerFlight(airline, flight, fixedtimestamp, {from: airline});
    ...
    assert.equal(await config.flightSuretyData.isFlightRegistered.call(airline, flight, fixedtimestamp), true, "Flight should be registered now!");
  ...
  it('can buy insurance', async () => {
    ...
    assert.equal(await config.flightSuretyData.isFlightRegistered.call(airline, flight, fixedtimestamp), true, "Flight should be registered now!");
    await config.flightSuretyApp.buy(airline, flight, fixedtimestamp, {value: web3.toWei("1", "ether"), from: passenger});
  ...
  it('can request flight status', async () => {
    ...
    let r = await config.flightSuretyApp.fetchFlightStatus(airline, flight, fixedtimestamp);
    //console.log(`\n\nindex: ${r}`);
    ...
    for(let a=6; a<TEST_ORACLES_COUNT+6; a++) {
      ...
      for(let idx=0;idx<3;idx++) {
        ...
        try {
          ...
          //if(oracleIndexes[idx].toNumber()==r.toNumber()){
            //console.log(`\n\ncurrent: ${oracleIndexes[idx]}`);
            let x = await config.flightSuretyApp.submitOracleResponse(oracleIndexes[idx], airline, flight, fixedtimestamp, 20, { from: accounts[a] });
            ...
            let flightStatus = await config.flightSuretyApp.viewFlightStatus.call(airline, flight, fixedtimestamp);
            console.log('\nPost', accounts[a], idx, oracleIndexes[idx].toNumber(), airline, flight, fixedtimestamp, flightStatus.toNumber());
      ...
      /*var event1 = config.flightSuretyApp.FlightStatusInfo();
      await event1.watch((err, res) => {
        console.log(`\n\nFlight Status Info: airline: ${res.args.airline}, flight:  ${res.args.flight}, timestamp: ${res.args.timestamp.toNumber()}, status: ${res.args.status.toNumber()}`);
      });*/
    }
